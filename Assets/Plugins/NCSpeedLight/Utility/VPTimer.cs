/*----------------------------------------------------------------
            ...
            // 
            // FileName: VPTimer.cs
			// Describle: ref from vp_timer
			// Created By:  Wells Hsu
			// Date&Time:  2016/4/5 9:31:31
            // Modify History:
            //
//----------------------------------------------------------------*/
...
namespace NCSpeedLight
{
    public class VPTimer : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            m_EventBatch = 0;
            while ((VPTimer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
            {
                ...
                if (m_EventIterator < 0)
                {
                    ...
                    m_EventIterator = VPTimer.m_Active.Count - 1;
                    break;
                }

                // prevent index out of bounds
                if (m_EventIterator > VPTimer.m_Active.Count - 1)
                    m_EventIterator = VPTimer.m_Active.Count - 1;

                // execute all due events
                if (Time.time >= VPTimer.m_Active[m_EventIterator].DueTime ||	// time is up
                    VPTimer.m_Active[m_EventIterator].Id == 0)					// event has been canceled ('Execute' will kill it)
                    VPTimer.m_Active[m_EventIterator].Execute();
                else
                {
                    // handle pausing
                    if (VPTimer.m_Active[m_EventIterator].Paused)
                        VPTimer.m_Active[m_EventIterator].DueTime += Time.deltaTime;
                    else
                        // log lifetime
                        VPTimer.m_Active[m_EventIterator].LifeTime += Time.deltaTime;
                }
        ...
        private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
        {
            ...
            {
                Helper.LogError("Error: (vp_Timer) Aborted event because function is null.");
                return;
            }
            ...
            VPTimer.m_EventCount++;
            m_NewEvent.Id = VPTimer.m_EventCount;
            ...
            VPTimer.m_Active.Add(m_NewEvent);
        ...
        /// </summary>
        private static void Cancel(VPTimer.Handle handle)
        {
        ...
        public static void CancelAll()
        {
            ...
            for (int t = VPTimer.m_Active.Count - 1; t > -1; t--)
            {
                VPTimer.m_Active[t].Id = 0;
            }
        ...
        public static void CancelAll(string methodName)
        {
            ...
            for (int t = VPTimer.m_Active.Count - 1; t > -1; t--)
            {
                if (VPTimer.m_Active[t].MethodName == methodName)
                    VPTimer.m_Active[t].Id = 0;
            }
        ...
        {
            VPTimer.m_Active.Clear();
            VPTimer.m_Pool.Clear();
        ...
        private class Event
        {
            ...
            private void Recycle()
            {
                ...
                if (VPTimer.m_Active.Remove(this))
                    m_Pool.Add(this);
            ...
            private void Destroy()
            {
                ...
                VPTimer.m_Active.Remove(this);
                VPTimer.m_Pool.Remove(this);
        ...
        public class Handle
        {
            ...
            private VPTimer.Event m_Event = null;	// timer we're pointing at
            ...
            public int Id
            {
                ...
                set
                {
                    ...
                    for (int t = VPTimer.m_Active.Count - 1; t > -1; t--)
                    {
                        if (VPTimer.m_Active[t].Id == m_Id)
                        {
                            m_Event = VPTimer.m_Active[t];
                            break;
                        }
                    }
                    if (m_Event == null)
                        Helper.LogError("Error: (vp_Timer.Handle) Failed to assign event with Id '" + m_Id + "'.");
