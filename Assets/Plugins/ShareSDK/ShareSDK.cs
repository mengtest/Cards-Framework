...
namespace cn.sharesdk.unity3d
{
	...
	public class ShareSDK : MonoBehaviour 
	{
		...
		{				
            ...
            for (int i = 0; i < devInfoFields.Length; i++)
            {
                FieldInfo devInfoField = devInfoFields[i];
                DevInfo info = (DevInfo)devInfoField.GetValue(devInfo);
                FieldInfo tempFi = info.GetType().GetField("type");
                if (tempFi == null)
                {
                    continue;
                }
                int platformId = (int)tempFi.GetValue(info);
