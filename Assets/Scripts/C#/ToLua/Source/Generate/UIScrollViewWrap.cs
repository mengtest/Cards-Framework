...
public class UIScrollViewWrap
{
	...
	static int RestrictWithinBounds(IntPtr L)
	{
		try
		{
			...
			if (count == 2 && TypeChecker.CheckTypes(L, 1, typeof(UIScrollView), typeof(bool)))
			{
			...
			else if (count == 4 && TypeChecker.CheckTypes(L, 1, typeof(UIScrollView), typeof(bool), typeof(bool), typeof(bool)))
			{
				UIScrollView obj = (UIScrollView)ToLua.ToObject(L, 1);
				bool arg0 = LuaDLL.lua_toboolean(L, 2);
	...
	static int UpdateScrollbars(IntPtr L)
	{
		try
		{
			...
			if (count == 1 && TypeChecker.CheckTypes(L, 1, typeof(UIScrollView)))
			{
				UIScrollView obj = (UIScrollView)ToLua.ToObject(L, 1);
				obj.UpdateScrollbars();
				return 0;
			}
			else if (count == 2 && TypeChecker.CheckTypes(L, 1, typeof(UIScrollView), typeof(bool)))
			{
				UIScrollView obj = (UIScrollView)ToLua.ToObject(L, 1);
				bool arg0 = LuaDLL.lua_toboolean(L, 2);
